...
class SwinTransformer(nn.Module):
    ...
    def freeze_backbone(self):
        backbone = [self.patch_embed, self.layers[:3]]
...
def swin_transformer_tiny(input_shape=[224, 224], pretrained=False, num_classes=1000):
    ...
    if pretrained:
        new_state_dict=torch.load("..\\model_data/swin_tiny_patch4_window7_224_imagenet1k.pth")
...
def swin_transformer_base(input_shape=[224, 224], pretrained=False, num_classes=1000):
    ...
    if num_classes!=1000:
        model.head = nn.Linear(model.num_features, num_classes)
    return model
# def swin_transformer_base_384(input_shape=[384, 384], pretrained=False, num_classes=1000):
#     model = SwinTransformer(input_shape, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], embed_dim=128, drop_path_rate=0.2)
#     if pretrained:
#         new_state_dict=torch.load("..\\model_data\\swin_base_patch4_window12_384.pth")["model"]
...
def swin_transformer_base_384(input_shape=[384, 384], pretrained=False, num_classes=1000):
    model = SwinTransformer(input_shape, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], embed_dim=128, drop_path_rate=0.2,window_size=12)
    if pretrained:
        new_state_dict=torch.load("..\\model_data\\swin_base_patch4_window12_384.pth")["model"]
        model.load_state_dict(new_state_dict, strict=False)
